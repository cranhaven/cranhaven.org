...
## Add 'check_url'
history2 <- history2 |>
  mutate("check results" = sprintf("https://cran-archive.r-project.org/web/checks/%s/%s_check_results_%s.html", sub("-.*", "", date), date, package))
...
history2 |> 
  reactable(
    columns = list(
      ...
      "check results" = colDef(cell = function(value) {
        htmltools::tags$a(href = value, target = "_blank", "checks")
      }),
