...
```{r return-time3}
#| fig-cap: "**Packages archived and date since the previous release.** The 
...
#| fig-cap: "**Resubmission delay: % spend in review after submitting a new version till it is accepted.** Only includes packages that were accepted one day later than submitted (to avoid dividing by 0)"
...
```{r r-versions}
#| fig-cap: "**More packages are archived the closer the release date is.**"
archived_all_versions |> 
  ggplot() +
...
```{r r-versions2}
#| fig-cap: "**Packages are usually archived weeks after a release.**"
archived_all_versions |> 
  ggplot() +
...
```{r r-versions3}
#| fig-cap: "**Packages tend to be archived right before a release or after it but not in the middle of releases.**"
...
```{r r- version4}
#| fig-cap: "**Trend by year of packcages being archived.** The closer to the left the closer they are after a release, the more to the right the closer they are archived before a R-release."
ggplot(archived_all_versions) +
...
```{r aged}
#| fig-cap: "**Time since being accepted to being archived.** Most packages are 
...
```{r aged-public}
#| fig-cap: "**Time since last version till the package is archived.** Most 
...
```{r aged-public1}
#| fig-cap: "**Age of packages being archived.** Older packages are increasingly being 
...
```{r aged-public2}
#| fig-cap: "**Time since previous releases of packages being archived.** Almost
...
```{r depending-on}
#| tbl-cap: "**Dependencies impact** Most packages archived result in another package archived."
...
```{r depending-on2}
#| fig-cap: "**Archived packages due to a dependency often come back.** In 
...
#| fig-cap: "**Packages with not responsive maintainer address are archived 
#|  later after the last version.** The line indicates the aproximation of these two variables."
archived_all_versions |> 
