...
```{r multiple-actions}
#| tbl-cap: "**Events per package in the same date.** Most packages have just 
...
This affects `r length(no_multiples_actions_pack)` packages out of `r n_distinct(pkg_history$package)` (`r percent(length(no_multiples_actions_pack)/n_distinct(pkg_history$package))`).
...
In total there are `r length(pkg_failing_qc)` different packages identified with problematic records/processing from `r n_distinct(pkg_history$pacakge)`. 
Some of them were found that had at two or more different issues `r tb`.
...
archived_all <- merge(times_unarchived, archived_resubmitted, all = TRUE,
  ...
  # 
         ...
         time_since_last_v = difftime(archived, previous_accepted, units = "weeks"),
...
```{r table-times}
#| tbl-cap: "**Table with summary statistics of time to get back to CRAN after 
#|  being archived.** Median time below 33 days."
...
```{r plot-times}
#| fig-cap: "**summary statistics of time to get back to CRAN after being 
#|  archived.** Same as the previous table but in graphical format."
archived_all_versions |> 
  ...
  scale_y_log10(labels = label_log(),
                ...
                sec.axis = dup_axis(breaks = c(1, 7, 15, 30*1:5),
                                    ...
                                    name = "days")) +
  theme_minimal() +
  labs(y = "Days till package is back on CRAN", 
       x = "Times a package has been archived",
       title = "Time per each ")
```
...
archived_all_versions |> 
  ...
  theme_minimal() +
  labs(title = "Most packages are back to CRAN soon",
...
```{r return-time2}
archived_all_versions |> 
  filter(!is.na(time_since_first), !package %in% all_failing_qc) |>
  ggplot() +
...
```{r archived-type}
#| fig-cap: "**Resubmission process by date of being archived.** Events in black
...
```{r archived2resubmit}
#| fig-cap: "**Submissions are slightly faster for those that are accepted.** Percentage of packages that are submitted before a given time."
resubm_t <- archived_all_versions |> 
  mutate(archived2resubmit = difftime(initial_resubmission, archived, units = "weeks"))  |> 
  filter(!is.na(archived2resubmit)) |> 
  ggplot() +
  stat_ecdf(aes(archived2resubmit, col = back_on_cran)) +
  scale_color_manual(values = legend) +
...
resubm_t_focus <- archived_all_versions |> 
  mutate(archived2resubmit = difftime(initial_resubmission, archived, units = "days"))  |> 
  filter(!is.na(archived2resubmit)) |> 
  ggplot() +
  stat_ecdf(aes(archived2resubmit, col = back_on_cran), show.legend = FALSE) +
  scale_color_manual(values = legend) +
  scale_y_continuous(labels = label_percent(), expand = expansion(c(0, 0.005), c(0, 0.005))) +
  scale_x_continuous(expand = expansion()) +
...
subm_rel |> 
  ggplot() +
  geom_histogram(aes(time_rel), fill = "green", bins = 50) +
...
But some (~ `r percent(subm_rel)`) spend most of the time trying to pass checks to comply with CRAN policies: maybe trying to fix issues detected on submission or waiting for CRAN maintainers feedback.
