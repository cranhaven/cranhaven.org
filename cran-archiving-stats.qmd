...
This happens for `r length(bad_start_packages)` packages.
...
no_muliples_actions_pack <- multiple_actions |> 
...
This affects `r length(no_muliples_actions_pack)` packages out of `r n_distinct(out$packages)`.
...
```{r qc}
individually <- length(no_muliples_actions_pack) +
  length(over_unarchived) +
  length(bad_start_packages)

pkg_failing_qc <- unique(c(no_muliples_actions_pack,
                      over_unarchived,
                      bad_start_packages))

tb <- table(table(c(no_muliples_actions_pack,
        over_unarchived,
        bad_start_packages)))
```
...
```{r return-time}
first_publ <- out |> 
  filter(!package %in% bad_start_packages) |> 
  summarise(.by = package,
...
ggplot(history_failed) +
  geom_histogram(aes(date, fill = back), col = "lightgray", bins = 8*12) +
...
# out_failed |> group_by(check) |> summarise(n = n_distinct(package))
# 
# no_muliples_actions_pack,
#         over_unarchived,
#         bad_start_packages

of <- out_failed |> 
  filter(!package %in% pkg_failing_qc) |> 
  arrange(package, date, desc(action)) |> 
