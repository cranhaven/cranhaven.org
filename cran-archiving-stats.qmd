...
if (!file.exists("20240519_cdh.RDS")) {
  ...
  cdh <- readRDS("20240519_cdh.RDS")
}
# Date we started recordding the CRAN submission queue.
...
archived <- out |> 
  group_by(package) |> 
  mutate(v_archived = cumsum(action == "archived"),
         v_accepted = cumsum(action == "accepted"),
         first_accepted = min(date[action == "accepted"]),
         last_accepted = max(date[action == "accepted" & lag(v_archived, default = max(v_archived)) == v_archived])) |>  
  group_by(package, v_accepted) |> 
  mutate(previous_version = max(date[action == "accepted"])) |> 
  ungroup() |> 
  group_by(package) |> 
  tidyr::fill(previous_version) |> 
  ungroup() |> 
  mutate(last_accepted = if_else(is.infinite(last_accepted), first_accepted, last_accepted),
         previous_version = if_else(is.infinite(previous_version), min(date), previous_version)) |> 
  filter(v_archived >= 1)
...
archived_resubmitted <- merge(archived, cran_submissions, all.x = TRUE) |> 
  ...
  summarise(.by = c(package, date),
            ...
            previous_version = unique(previous_version),
  ...
  mutate(initial_resubmission = if_else(
    ...
    time_since_first = difftime(date, first_accepted, units = "weeks"), 
    ) |> 
  ungroup() 
...
archived_all <- merge(times_unarchived, archived_resubmitted, all = TRUE,
  ...
  mutate(before_registry = is.na(initial_resubmission),
         back_on_cran = case_when(
           ...
           is.na(initial_resubmission) ~ "Never resubmitted",
           .default = "Unkown"),
...
archived_all_versions <- archived_all |> 
  ...
  mutate(date_r_rel = r_release(rver, archived), 
         ...
         time_since_previous = difftime(archived, previous_version, units = "weeks")
         ) |> 
  ungroup() |> 
...
archived_all_versions |> 
  ...
  summarise(.by = time_archived, 
            packages = n(), 
            min = min(archived2accepted, na.rm = TRUE), 
            q1 = quantile(archived2accepted, 0.25, na.rm = TRUE), 
            mean = mean(archived2accepted, na.rm = TRUE), 
            median = median(archived2accepted, na.rm = TRUE), 
            q3 = quantile(archived2accepted, 0.75, na.rm = TRUE), 
            max = max(archived2accepted, na.rm = TRUE)) |> 
...
archived_all_versions |> 
  ...
  ggplot() +
  stat_ecdf(aes(archived2accepted)) +
...
archived_all_versions |> 
  ...
  geom_segment(aes(y = package, x = accepted, xend = archived, col = archived2accepted)) +
...
archived_all_versions |> 
  ggplot() +
  geom_point(aes(time_since_first, time_since_previous, col = time_archived)) +
...
archived_all_versions |> 
  ggplot() +
  geom_point(aes(first_accepted, archived2accepted, col = time_archived)) +
  geom_abline(intercept = Sys.Date(), slope = -2) +
  scale_y_continuous(expand = expansion(mult = c(0, NA), add = c(0, NA)), 
                     sec.axis = dup_axis()) +
  theme_minimal() +
...
archived_all_versions |> 
  ggplot() +
  geom_histogram(aes(archived2accepted), binwidth = 4) +
...
archived_all_versions |> 
  ggplot() +
  geom_count(aes(archived, archived2accepted)) +
  geom_abline(slope = -1, intercept = Sys.Date(), linetype = 2, col = "gray") +
  geom_rug(aes(archived, archived2accepted), sides = "b", outside = TRUE, length = unit(0.015, "npc"), 
           col = "gray") +
  theme_minimal() +
...
archived_all_versions |> 
  ggplot() +
  geom_histogram(aes(archived2accepted), binwidth = 7) +
  theme_minimal() +
  scale_y_continuous(expand = expansion(c(0, NA), c(0, NA))) +
...
packages_archived <- archived_all_versions |> 
  filter(.by = package, any(time_archived > 1))
...
sample_size <- sum(archived_all$archived > start_archive)/nrow(archived_all)
archived_all |> 
  filter(!package %in% pkg_failing_qc) |> 
  # filter(archived >= as.Date("2020-09-12")) |> 
  count(before_registry, 
        new_submission = !is.na(initial_resubmission), 
        new_version = !is.na(accepted), sort = TRUE,
        name = "events") |> 
...
Based on a the latest data available which is `r percent(sample_size)` of the archived packages. Most packages try to get back on CRAN. 
Those that try slightly above half of them are included back again.

But how fast is the process of being back on CRAN?

```{r speed-resubmission}
archived_all_versions |> 
  filter(!is.na(archived2accepted) & !is.na(delay_submission)) |> 
  mutate(time_rel = as.numeric(archived2accepted - delay_submission)/as.numeric(archived2accepted),
...
```{r tbl-not-fixed}
#| tbl-cap: "**Tally of reasons of packages not fixed**. Many packages are not 
#|  fixed and the maintainer address."
archived_all_versions |> 
  filter(!is.na(not_fixed) & !requested) |>
  count(not_fixed, maintainer_address, policy_violation, sort = TRUE) |> 
  knitr::kable(align = "c")
```
...
archived_all_versions |> 
  ggplot() +
  geom_histogram(aes(time_since_rel, fill = back_on_cran), col = "lightgray", bins = 54) +
  scale_y_continuous(expand = expansion(add = 0, mult = c(0, NA)),
                     sec.axis = dup_axis()) +
  scale_x_continuous(expand = expansion()) +
  scale_fill_manual(values = legend) + 
...
ggplot(archived_all_versions) +
  ...
  annotate("text", x = c(-45, 45), y = 610, label = c("Next release", "Previous release")) +
...
impossible_dates <- archived_all_versions |> 
  filter(time_since_first <= 0 & time_since_previous > 50) |> 
  pull(package) |> 
...
archived_all_versions |> 
  ...
  ggplot() +
  geom_count(aes(time_since_first, time_since_previous))
archived_all_versions |> 
  filter(!package %in% c(impossible_dates, pkg_failing_qc)) |> 
  ggplot() +
  geom_histogram(aes(time_since_first), bins = 52*23) +
...
tsp <- archived_all_versions |> 
  filter(!package %in% c(impossible_dates, pkg_failing_qc)) |> 
  ggplot() +
  geom_histogram(aes(time_since_previous), bins = 52*27) +
  scale_x_continuous(expand = expansion(), breaks = seq(0, 1200, 52)) +
  scale_y_continuous(expand = expansion(), breaks = seq(0, 1200, 50)) +
  theme_minimal()
tsp_inset <- archived_all_versions |> 
  filter(!package %in% c(impossible_dates, pkg_failing_qc),
         time_since_previous <= 52) |> 
  ggplot() +
  geom_histogram(aes(time_since_previous), bins = 52) +
  scale_x_continuous(expand = expansion(), breaks = seq(0, 1200, 4)) +
  scale_y_continuous(expand = expansion(), breaks = seq(0, 1200, 50)) +
  theme_minimal() +
...
tsp + 
  ...
  inset_element(tsp_inset, 0.2, 0.2, 1, 1) & 
  labs(x = "Time since last version (weeks)")

archived_all_versions |> 
  filter(!package %in% c(impossible_dates, pkg_failing_qc)) |> 
  ggplot(aes(archived, time_since_first)) +
...
archived_all_versions |> 
  filter(!is.na(dp)) |> 
  count(dp, sort = TRUE, name = "Affected packages") |> 
...
archived_all_versions |> 
  filter(!is.na(dp)) |> 
  count(archived, back_on_cran, name = "events", sort = TRUE) |>
  ggplot() +
  geom_histogram(aes(events, fill = back_on_cran), bins = 10) +
  scale_fill_manual(values = legend) +
  theme_minimal() +
  scale_y_continuous(expand = expansion(add = c(0, NA), mult = c(0, NA)))
```
...
archived_all_versions |> 
  ...
  labs(x = "Date package was archived",
       ...
       size = "Evenets",
       title = "Maintainers are now more responsive")
```
